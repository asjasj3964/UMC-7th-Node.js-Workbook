...
// 특정 회원 정보 조회
export const getMember = async (memberId: bigint) => {
    try{
        ...
        //     //inactiveAt: member.inactiveAt ? member.inactiveAt.toISOString() : null
        // };
        return member;
    }
    catch(err){
...
// 특정 회원의 모든 리뷰 조회
export const getAllReviews = async(memberId: bigint, cursor: number) => {
    try{
