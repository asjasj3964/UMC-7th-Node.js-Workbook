...
import { bodyToRestaurant } from "../dtos/restaurant.dto.ts";
import { restaurantRegist } from "../services/restaurant.service.ts"
import { listRestaurantReviews, listRestaurantMissions } from "../services/restaurant.service.ts";
import { NotExistError } from "../errors.ts";
import { Response, Request, NextFunction } from 'express';

// 식당 등록 핸들러
export const handleRestaurantRegist = async(req: Request, res: Response, next: NextFunction) => {
    /*
...
export const handleListRestaurantReviews = async(req: Request, res: Response, next: NextFunction) => {
    ...
    */
    const reviews = await listRestaurantReviews(
        BigInt(req.params.restaurantId), // URL 경로에서 restaurantId(Path Parameter)를 가져온다.
...
// 특정 식당 모든 미션 조회 핸들러
export const handleListRestaurantMissions = async(req: Request, res: Response, next: NextFunction) => {
    /*
    #swagger.ignore = false
    #swagger.tags = ['restaurant-controller']
    ...
    */
    const missions = await listRestaurantMissions(
        BigInt(req.params.restaurantId),
        typeof req.query.cursor === "string"? parseInt(req.query.cursor) : 0
    );
